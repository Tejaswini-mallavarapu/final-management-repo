...
import { BrandownerData } from "../../../../data/productsData";
...
const Brandowner = () => {
    ...
    const renderRow=(item,index)=>(
        <tr key={`row-${item.id}-${index}`}>
            ...
            <td><img src={item.images[0]} alt="logo" className="brandowner-logo"/></td>
            <td>{item.Name}</td>
            <td>{item.category}</td>
    ...
    const filteredData=BrandownerData.filter((item)=>{
        return (
            (filter.BrandownerName==="All" || item.Name===filter.BrandownerName) &&
            (filter.productscategory==="All" || item.category===filter.productscategory) &&
            (filter.productsubcategory==="All" || item.subCategory===filter.productsubcategory) &&
        ...
        <div className='management-products-container'>
            <div className="management-products-page">
                ...
                <div className={`filter-boxes ${showFilters ? "open" : ""}`}>
                    ...
                    <CustomSelect className="filter-select filter-manu"
                        label="Brand Owner Name"
                        options={["All","Sri Animalife Biotech","Unique Bio Minerals"]}
                        value={filter.BrandownerName}
                        onChange={(value)=>setFilter({...filter,BrandownerName:value})}
                    />
                    <CustomSelect className="filter-select filter-manu"
                        label="Product Category"
                        options={["All","Aquaculture","Agriculture","Human Medicine"]}
                        value={filter.productscategory}
                        onChange={(value)=>setFilter({...filter,productscategory:value})}
                    />
                    <CustomSelect
                        ...
                        onChange={(val) =>
                        setFilter({ ...filter, productsubcategory: val })}
                    />
                    <CustomSelect
                        className="filter-select filter-manu"
                        label="Status"
