...
import { SignupData } from "../../data/productsData";
...
const SignUpRequest = () => {
    ...
    const filterConfig=[
        ...
        },
        {
            ...
            label:"Status",
            type:"select",
            options:["All","New","Rejected","Approved"]
        }
    ];
    ...
    }=usePagination(SignupData,itemsPerPage);
    ...
    const renderTickets=(item,index)=>(
        <tr key={item.id}>
        <td className="id">{totalItems-((currentPage-1)*itemsPerPage + index)}</td>
        <td>{item.Date}</td>
        <td>{item.Name}</td>
        <td>{item.Role}</td>
        <td>{item.Userid}</td>
        <td>{item.Category}</td>
        <td className="sub-width">{item.SubCategory}</td>
        <td className="status-col"><span className={`status ${
        ...
        <div className="signup-cnt">
            ...
            <div className="management-products-container">
                ...
                <ReusableTable
                    columns={columns}
                    data={currentData}
                    renderRow={renderTickets}
                    emptyState={{
                        title:"No Tickets found in the list.",
                        description:"No products available. Please add a product to continue.",
                                }}

                />
                {
                    SignupData.length>itemsPerPage && (
                        <Pagination
