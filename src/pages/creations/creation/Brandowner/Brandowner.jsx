...
const Brandowner = () => {
    ...
    const renderRow = (item, index) => (
        ...
        <td>
            <img src={item.images[0]} alt="logo" className="brandowner-logo" />
        </td>
        <td>{item.Name}</td>
        <td>{item.category}</td>
    ...
    const filteredData = BrandownerData.filter((item) => {
        ...
        (filter.BrandownerName === "All" ||
            item.Name === filter.BrandownerName) &&
        (filter.productscategory === "All" ||
            item.category === filter.productscategory) &&
        (filter.productsubcategory === "All" ||
            item.subCategory === filter.productsubcategory) &&
    ...
    return (
        ...
        <ReusableTable
            ...
            emptyState={{
                image:Images.creationempty,
                title: "No Brand Owners found in the list.",
                description: "No brand owners available. Please add a brand owner to continue.",
                buttonText:"Create a Brand Owner",
            }}
        />
        </div>
    );
